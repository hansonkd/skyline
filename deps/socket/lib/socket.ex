  ...
  * `udp://host:port` for Socket:UDP
  ...
  def listen(%URI{scheme: "tcp", host: host, port: port}) do
    Socket.TCP.listen(
      port,
      local: [
        address: if(host == "*", do: "0.0.0.0", else: host)
      ]
    )
  end
...
defimpl Socket.Protocol, for: Port do
  def equal?(self, other) when other
                               |> is_port do
    self == other
  end
...
defimpl Socket.Protocol, for: Tuple do
  ...
  def equal?(self, other) when self
                               |> Record.is_record(:sslsocket) and other
                                                                   |> Record.is_record(:sslsocket) do
    self == other
  end
  ...
  def accept(self, options \\ []) when self
                                       |> Record.is_record(:sslsocket) do
    Socket.SSL.accept(self, options)
  end

  def options(self, opts) when self
                               |> Record.is_record(:sslsocket) do
    Socket.SSL.options(self, opts)
  end

  def packet(self, type) when self
                              |> Record.is_record(:sslsocket) do
    :ssl.setopts(self, packet: type)
  end

  def process(self, pid) when self
                              |> Record.is_record(:sslsocket) do
    :ssl.controlling_process(self, pid)
  end

  def active(self) when self
                        |> Record.is_record(:sslsocket) do
    :ssl.setopts(self, active: true)
  end

  def active(self, :once) when self
                               |> Record.is_record(:sslsocket) do
    :ssl.setopts(self, active: :once)
  end

  def passive(self) when self
                         |> Record.is_record(:sslsocket) do
    :ssl.setopts(self, active: false)
  end

  def local(self) when self
                       |> Record.is_record(:sslsocket) do
    :ssl.sockname(self)
  end

  def remote(self) when self
                        |> Record.is_record(:sslsocket) do
    :ssl.peername(self)
  end

  def close(self) when self
                       |> Record.is_record(:sslsocket) do
    :ssl.close(self)
  end
end
