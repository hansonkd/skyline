defmodule Spotmq.Msg.Decode.Utils do
  ...
  alias Spotmq.Msg
    ...
    def decode(msg = <<_m :: size(16)>>, socket, transport) do
      header = decode_fixheader(msg, socket, transport)
      var_m = read_bytes(socket, transport, header.length)
      decode_message(var_m, header)
    end

    def decode_fixheader(<<type :: size(4), dup :: size(1), qos :: size(2),
                 retain :: size(1), len :: size(8)>>, socket, transport) do
      FixedHeader.create(
        binary_to_msg_type(type),
        (dup == 1),
        binary_to_qos(qos),
        (retain == 1),
        binary_to_length(<<len>>, socket, transport)
      )
    end
  ...
  def binary_to_length(_bin, count = 0, _readByte, _t) do
    raise "Invalid length"
  end
  def binary_to_length(<<overflow :: size(1), len :: size(7)>>, count \\ 4, socket, transport) do
    case overflow do
      1 ->
        byte = read_bytes(socket, transport, 1)
  ...
  defp read_bytes(socket, transport, 0), do: ""
  defp read_bytes(socket, transport, nr) do
      result = case transport.recv(socket, nr, :infinity) do
        {:ok, bytes} -> bytes
